Progress = {
    stats = {}, -- [citizenid] = { name, sold, earned, updated }
}

local KVP_KEY = 'djdrugs_progress_v1'
local dirty = false
...
function Progress.GetRow(citizenid)
    if not citizenid then return nil end
    return Progress.stats[citizenid]
end
...
lib.callback.register('djdrugs:server:getProgressBoard', function(source)
    if not enabled() then
        return nil
    end
...
CreateThread(function()
    loadStats()
    Utils.Debug('progress loaded', tostring(Progress.stats and 'ok' or 'empty'))
end)
