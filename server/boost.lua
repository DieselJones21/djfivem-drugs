Boost = {
    sell = nil,    -- { multiplier, endsAt, startedBy, label }
    harvest = nil, -- { multiplier, endsAt, startedBy, label }
}
...
function Boost.Start(kind, multiplier, durationSeconds, adminName)
    ...
    TriggerClientEvent('djdrugs:client:boostUpdated', -1, Boost.GetState())
    return true
end
...
lib.callback.register('djdrugs:server:getBoostState', function()
    return Boost.GetState()
end)

lib.callback.register('djdrugs:server:canManageBoost', function(source)
    return Bridge.IsBoostAdmin(source)
end)

RegisterNetEvent('djdrugs:server:startBoost', function(kind, multiplier, durationSeconds)
    local src = source
...
RegisterNetEvent('djdrugs:server:stopBoost', function(kind)
    local src = source
    if not Bridge.IsBoostAdmin(src) then
        Server.Notify(src, 'No permission', 'error')
        return
