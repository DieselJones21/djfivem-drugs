lib.callback.register('djdrugs:server:canProcess', function(source, drugId)
...
RegisterNetEvent('djdrugs:server:process', function(drugId)
    local src = source
    local drug = Utils.GetDrug(drugId)
    ...
    if Server.AddItem(src, outItem, outAmount) then
        ...
        -- Refund ingredients if add failed
        for i = 1, #drug.ingredients do
            local ing = drug.ingredients[i]
